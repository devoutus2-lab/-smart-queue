...
import { ChevronLeft, ChevronRight, LogOut, Presentation } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-soft-gradient">
      ...
      <main className={`dashboard-frame relative ${sidebarHidden ? "" : "lg:grid-cols-[320px_minmax(0,1fr)] xl:grid-cols-[340px_minmax(0,1fr)]"}`}>
        {!sidebarHidden ? (
          <aside className="glass-panel compact-target relative rounded-[2.1rem] border border-white/60 p-6 shadow-luxury dark:border-slate-800 lg:sticky lg:top-24 lg:self-start xl:p-8">
            <button
        ...
        <section className={`min-w-0 space-y-8 xl:space-y-10 ${sidebarHidden ? "lg:col-span-full" : ""}`}>
          {sidebarHidden ? (
            <button
              ...
              className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 bg-white/90 text-slate-600 shadow-sm transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900/90 dark:text-slate-200 dark:hover:bg-slate-800"
